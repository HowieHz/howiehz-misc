...
- [controllers/](../../../tools/graphwar-killer/controllers/) contains page controllers: `agent/` for Agent reads, manual debug files, and clear-failure auto exports, `debug/` for debug activation and timings, `screenshot/` for screenshot input, `settings/` for input validation, `detection/` for detection workflows, `path/` for path state and editing,
  `pathfinding/` for pathfinding sessions, `stage/` for stage interactions, and `result/` for result actions.
