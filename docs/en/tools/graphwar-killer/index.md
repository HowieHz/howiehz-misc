...
<table tabindex="0">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Replay the current path to its actual recovery point → scan the current height to its first blocked column → generate right-gate control points from every free y beyond that column, preferring the candidate whose next horizontal run reaches farthest; every intermediate gate lies strictly between the current edge start and target x → tunnel vertically near the left gate and replay each candidate's final formula from the launch point → continue from the first real accepted point with <code>x≥R</code> → once the target is horizontally reachable, append the assigned target as the final control point; succeed only when the full replay hits the target and reaches that control-point x, otherwise keep exhausting branches → when Fast Mode is off, try point deletion and fully replay each deletion candidate → update the path. Fast Mode does not change scanning; it only skips point deletion.</td>
    </tr>
    <tr>
      <td>
        <table class="graphwar-workflow-settings" role="presentation">
    ...
    <tr>
      ...
      <td>Check the current path → group soldiers by center x and allocate strictly increasing hit-circle control points to equal-x soldiers → process targets by increasing allocated x, using the same single-target glitch scan for every edge → after a hit, commit the whole edge ending exactly at the assigned target and make it the new fixed prefix; after exhausting every y/window branch without a path, permanently skip that target and try the next target from the same prefix → reuse the fixed prefix's formula, full trajectory, actual recovery point, and committed-target hit result; discard and rebuild that cache after committing a new edge, while still replaying every appended candidate's final formula from the launch point → when Fast Mode is off, try deleting intermediate points while preserving committed target anchors → use one final formula replay to confirm the hit sequence, last anchor x, and every actual hit at once; an incidentally hit skipped target is reported, but never added to the fixed required sequence or retried.</td>
    </tr>
  </tbody>
</table>
...
[^routing-cache]: When the obstacles, `x+` direction, and related tolerances stay unchanged, prepared obstacle data is reused instead of rebuilt. The cache affects speed only, not route selection. One-Click Clear reuses it only within the current run, with each parallel task keeping its own copy. When ordinary One-Click Clear disables a failed edge and selects another route, it also reuses the validated edge prefix that is exactly shared with the previous attempt; edges after the divergence are still simulated normally. Glitch One-Click Clear keeps exactly one cache for the current fixed prefix: failed targets reuse it, while committing a new edge discards it. This cache never replaces the final-formula replay for an appended candidate.
