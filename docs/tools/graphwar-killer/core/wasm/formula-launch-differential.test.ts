...
describe("Graphwar WASM cold formula launch differential", () => {
  ...
  it.each([
    ...
    async ({ equation, formulaPathSteepness, steepness }) => {
      ...
      it("matches a Step-glitch mask winner", async () => {
        ...
        expect(
          oracle.context.compiledMaterials.stepFormula?.terms.some((term) => term.glitchSegment !== undefined),
        ).toBe(true);
  ...
  function expectFloatEquivalent(actual: number, expected: number, label: string, maxUlpDistance = 64n) {
