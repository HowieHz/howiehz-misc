...
“结果转分数”只改变结果显示、复制和 Agent 发射使用的公式，不改变内部求解与轨迹回放。工具会优先输出最简分数，并按 Graphwar 分别解析分子、分母为 `double` 后再相除的规则验证运行值；无法保证等价的小数会保留原样并显示提示。同一结果的显示、复制、手动发射和托管发射使用相同的对外公式规则。
...
#### 一键清图目标分配 {#graphwar-killer-pathfinding-targets}

- 所有一键清图模式优先使用士兵圆心；圆心不在当前路径尾点的 `x+` 侧时，改用严格位于命中圆和可用边界内的最外侧整数截图像素。
- 初始目标 x 相同时，按截图像素 y 从大到小分配从 `x-` 到 `x+` 的命中圈位置；相同 y 保持识别输入顺序。每个士兵只使用自己的安全区间，无法继续前进时保留初始落点。
- 普通模式按最终目标 x 建立有向无环图；最终 x 相同的目标作为同层替代候选，彼此不建立违反严格 `x+` 的边。
- 邪道一键清图按最终目标 x 逐层扫描；同层候选从同一条已提交前缀依次尝试，首个成功目标成为该层控制点。
- 所有结果都会通过完整轨迹验证后再更新路径，未作为控制点但被弹道顺路命中的士兵仍计入击杀。
