...
<table tabindex="0">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>先把目标直接追加到当前路径，并从发射点完整回放最终整式；成功时立即更新路径 → 直连失败后，当前路径与最近成功整式完全一致时复用其真实恢复点，否则完整回放旧整式一次，同时确认已提交士兵和当前尾点 → 沿当前高度扫描到首个阻挡列 → 从阻挡列右侧的全部自由 y 中按“后续可达 x 最远”优先生成右门控制点；中间门严格位于当前边起点和目标 x 之间 → 在左门附近完成纵向隧穿，并把每个新候选的最终公式从发射点完整回放；已经失败的相同直连路径不会重复回放 → 从首个 <code>x≥R</code> 的真实接受点继续扫描 → 水平可达目标后把分配目标点作为最后控制点；只有整式回放命中当前目标和全部已提交士兵，并到达该控制点 x 才成功，历史士兵不要求保持旧命中顺序，否则继续穷尽分支 → 关闭快速模式时尝试删点，并完整回放每个删点候选 → 更新路径并保存这条最终整式的恢复点证据。快速模式不改变扫描算法，只跳过删点优化。</td>
    </tr>
    <tr>
      <td>
        <table class="graphwar-workflow-settings" role="presentation">
    ...
    <tr>
      ...
      <td>从当前路径尾点开始，按中心 x 分组并为同 x 士兵分配严格递增的命中圈控制点 → 按分配后的 x 从小到大处理新目标，每条边使用同一套单目标邪道扫描 → 命中后提交以分配目标点精确收尾的整条边，并直接把本次成功回放的真实恢复点提升为下一固定前缀；穷尽当前目标的全部 y/窗口分支仍无路时永久跳过，从同一前缀尝试下一个更右目标 → 每个追加候选仍从发射点完整回放最终整式，失败目标不会破坏最近成功前缀的证据 → 关闭快速模式时尝试删除中间点，但保留已提交目标锚点；每次接受删除都改用删点后精确路径的证据 → 最终一次整式回放同时确认全部选中新目标、全部旧目标、最后锚点 x 和所有实际命中 → 原子写回路径与实际命中的士兵序列；显式目标保留仍存在的控制点锚点，顺路命中保存为无锚点目标，后续追加继续保证这些士兵不会丢失，但不强制沿用旧命中顺序。</td>
    </tr>
  </tbody>
</table>
...
[^routing-cache]: 障碍、`x+` 方向和相关容差不变时，会复用已经处理好的障碍数据，省去重复准备；缓存只影响速度，不改变路线。一键清图只在本次任务内复用几何建路数据，并行任务各自保存一份。普通一键清图禁用失败边并重选路线时，还会复用与上一轮完全相同的已验证边前缀，分歧后的边仍按原规则模拟。邪道模式只保留最近一条成功最终整式的精确证据；路径、目标序列、坐标边界、公式设置、模拟容差或障碍快照任一不同都会重新回放。单目标与一键清图可以共享完全匹配的证据；一键清图在请求内成功提交新边时直接提升局部前缀，只有最终整路成功才发布给后续请求。该证据只省去相同旧整式的重复回放，不会替代新增后缀最终整式从发射点开始的完整回放。
