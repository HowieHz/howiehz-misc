...
开启“使用 Agent”和“智能寻路”后，只有当前算法在 `y`、`y'`、`y''` 三种游戏模式下都支持一键清图时才能开启托管；目前满足条件的是阶跃算法。托管会锁定所有计算和发射输入，每秒读取一次权威状态，在房间内自动准备，并只在本地真人玩家的当前回合计算一键清图。
...
<table tabindex="0">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>检查当前路径 → 收集 <code>x+</code> 方向的士兵中心 → 尝试起点到各目标、各目标之间的绕障路线 → 选择预计能命中最多士兵的顺序 → 逐段生成函数并模拟 → 排除模拟失败的连接并重新选择 → 删除不必要的路径点 → 最终一次完整回放同时复验目标顺序并统计所有实际命中 → 更新路径。</td>
    </tr>
    <tr>
      <td>
        <table class="graphwar-workflow-settings" role="presentation">
      ...
      <td>检查当前阶跃路径的实际落点 → 同一士兵因前面路线不同而产生多个实际落点时，分别继续尝试 → 逐段检查能否避开障碍并接上前一段 → 选择预计能命中最多士兵的顺序 → 从发射点生成并模拟完整函数 → 排除失败的连接并重新选择 → 删除不必要的路径点 → 最终一次完整回放同时复验目标顺序并统计所有实际命中 → 更新路径。</td>
    </tr>
    <tr>
      <td>
        <table class="graphwar-workflow-settings" role="presentation">
      ...
      <td>从当前路径尾点开始，按中心 x 分组并为同 x 士兵分配严格递增的命中圈控制点 → 按分配后的 x 从小到大处理新目标，每条边使用同一套单目标邪道扫描 → 命中后提交以分配目标点精确收尾的整条边，并直接把本次成功回放的真实恢复点和公式求解结果提升为下一固定前缀；后续公式只重新求解新增段，但仍从发射点完整回放最终整式 → 穷尽当前目标的全部 y/窗口分支仍无路时永久跳过，从同一前缀尝试下一个更右目标；失败目标不会破坏最近成功前缀的证据 → 关闭快速模式时尝试删除中间点，但保留已提交目标锚点；每次接受删除都改用删点后精确路径的证据 → 最终一次整式回放同时确认全部选中新目标、全部旧目标、最后锚点 x 和所有实际命中 → 原子写回路径与实际命中的士兵序列；显式目标保留仍存在的控制点锚点，顺路命中保存为无锚点目标，后续追加继续保证这些士兵不会丢失，但不强制沿用旧命中顺序。</td>
    </tr>
  </tbody>
</table>
