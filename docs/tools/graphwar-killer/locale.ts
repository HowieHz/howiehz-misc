...
export const graphwarKillerLocale = {
  equationModes: [
    {
      ...
      formulaPrefix: "y=",
      description: "输出阶跃函数",
      title: "按 Graphwar 的 y 模式输出或模拟普通函数",
    },
    {
      value: "dy",
      label: "y'",
  ...
  ui: {
    ...
    pathfinding: {
      ...
      obstacleExpansionTitle: "调整识别出的障碍和边界在寻路、模拟碰撞时的安全距离；邪道模式生效时优先使用邪道配置",
    ...
    settings: {
      ...
      actionBar: {
        ...
        liveClickPreviewWorkerCountTitle:
          "实时预览会用常驻工作线程并行计算；新鼠标位置只保留一个最新等待任务，已开始的任务算完后按请求顺序丢弃旧结果。默认 4，取值 1 到 16",
      },
      ...
      overflowProtectionTitle:
        "检查阶跃导数项在从发射点到 x+ 边界的可能采样范围内是否会发生指数溢出；会溢出时改用数值稳定形式",
      ...
      pathfinding: {
        ...
        workerCountTitle:
          "普通寻路几何搜索会放到工作线程；一键清图有向无环图建边最多同时运行这些边搜索工作线程。默认 4，取值 1 到 128；1 表示串行",
      },
      recognition: {
        ...
        templateMatchingWorkerCountTitle:
          "士兵模板匹配阶段同时运行的工作线程数量；默认 4，取值 1 到 128，实际不会超过候选数量",
      },
      simulator: "模拟器设定",
      ...
      stepSteepnessTitle: "阶跃函数的陡峭度参数 a；数值越大，拐点越陡，导数指数项也会在更短的 x 距离内达到溢出阈值",
      title: "设定",
    },
  },
