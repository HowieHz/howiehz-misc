...
export function formatSvgPolylinePoints(points: readonly PixelPoint[]) {
  return formatSvgPolylinePointSlice(points, points.length, 2);
}
...
export function formatVisibleTrajectoryPoints(points: readonly PixelPoint[], hitIndex: number) {
  ...
  return formatSvgPolylinePointSlice(points, visiblePointCount, 1);
}

function formatSvgPolylinePointSlice(points: readonly PixelPoint[], pointCount: number, minimumPointCount: number) {
