...
import type { GraphwarTrajectoryCalculationWorkerResponse } from "../../controllers/path/trajectory-calculation";
...
const postMessage = vi.fn<(message: GraphwarTrajectoryCalculationWorkerResponse) => void>();
...
describe("Trajectory Worker request boundary", () => {
...
function dispatch(request: unknown) {
  if (!handleMessage) {
    throw new Error("Trajectory worker message handler was not registered");
  }
