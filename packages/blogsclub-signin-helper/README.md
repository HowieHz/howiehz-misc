...
BlogsClub Check-in Assistant is a userscript that can:

- check your BlogsClub check-in status in the background;
- run an on-demand status check and open the Geetest challenge;
- submit the check-in after you complete the challenge;
- show a success message and today's check-in ranking;
- prepare a challenge before midnight in optional Rush Check-in mode;
...
- Helps prevent missed check-ins by checking the status and notifying you;
- Reduces unnecessary requests and lowers the load on the BlogsClub server.

## Quick Start

1. Install [Tampermonkey](https://www.tampermonkey.net), [Violentmonkey](https://violentmonkey.github.io), [Greasemonkey](https://www.greasespot.net), [ScriptCat](https://docs.scriptcat.org), or another compatible userscript manager.
2. Install this script.
3. By default, checks run only on BlogsClub pages. Open `https://www.blogsclub.org/` or another BlogsClub page.
4. Open your userscript manager menu and choose `BlogsClub 账号：未设置`.
...
7. To check immediately, choose `立即检查/签到`.

When the “Only check on BlogsClub pages” setting is enabled and you are on another site, a manual action tells you to switch to a BlogsClub page.
