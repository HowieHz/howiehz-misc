...
完整的行為說明、選單設定、網路請求、隱私提示、故障排除和已知限制，請閱讀[簡體中文文件](https://howiehz.top/misc/blogsclub-signin-helper/)或[英文文件](https://howiehz.top/misc/en/blogsclub-signin-helper/)。

## 快速開始

1. 安裝相容的使用者腳本管理器。
2. 開啟 [Greasy Fork 上的 BlogsClub 簽到助手](https://greasyfork.org/scripts/590073-blogsclub-check-in-helper) 並安裝。
...
預設只在 BlogsClub 頁面檢查。密碼儲存在使用者腳本管理器的本機儲存中，只會傳送到 BlogsClub 登入介面。
