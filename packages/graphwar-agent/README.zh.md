...
它不会修改官方客户端。它会把当前对局里正在渲染的士兵坐标、障碍数据和函数提交能力，暴露成只监听本机的 HTTP 接口。
...
- **静默原版预期噪声**：官方代码在正常取消开局倒计时、正常关闭对局 socket 时会打印异常栈。agent 只移除这些预期日志，不改变原版逻辑。
...
比较时会忽略源码 commit、commit 时间和 jar 生成环境这类构建信息。
...
- Agent 不做 JVMTI。唯一的字节码补丁只用于移除官方客户端的预期异常噪声。
